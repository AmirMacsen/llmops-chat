...
from internal.schema.segment_schema import GetSegmentWithPageRequest, GetSegmentWithPageResponse, GetSegmentResponse, \
    UpdateSegmentEnabledRequest, CreateSegmentRequest, UpdateSegmentRequest
...
class SegmentHandler:
    ...
    def get_segment_with_page(self, dataset_id: UUID, document_id: UUID):
        """获取指定知识库文档的片段列表"""
        # 提取请求并校验
        req = GetSegmentWithPageRequest(request.args)
        if not req.validate():

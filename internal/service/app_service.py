...
from internal.exception import NotFoundException, ForbiddenException, ValidationException, FailedException
...
class AppService(BaseService):
    ...
    def cancel_publish_app_config(self, app_id: UUID, account: Account) -> App:
        ...
        if app.status != AppStatus.PUBLISHED:
            raise FailedException("当前应用未发布，请核实后重试")
    ...
    def get_debug_conversation_summary(self, app_id: UUID, account: Account) -> str:
        ...
        if draft_app_config["long_term_memory"]["enable"] is False:
            raise FailedException("该应用并未开启长期记忆，无法获取")
    ...
    def debug_chat(self, app_id: UUID, query: str, account: Account) -> Generator:
        ...
        for agent_thought in agent.stream({
            ...
            # 将数据填充到agent_thought中，便于存储到数据库服务中
            if agent_thought.event != QueueEvent.PING:
                # 除了agent_message数据叠加，其他均为覆盖
                    ...
                    if event_id not in agent_thoughts:
                        ...
                        agent_thoughts[event_id] = {
                            **agent_thoughts[event_id],
                            ...
                            "latency": agent_thoughts[event_id]["latency"] + agent_thought.latency,
                        }
                else:
                    agent_thoughts[event_id] = {
                        "id": event_id,
    ...
    def _save_agent_thoughts(
        ...
        with flask_app.app_context():
            position = 0
            latency = 0
            # 在子线程中重新查询conversation和message
            conversation = self.get(Conversation, conversation_id)
            message = self.get(Message, message_id)
            for key, item in agent_thoughts.items():
                # 存储长期记忆召回、推理以及消息、动作、知识库检索
                if item["event"] in [

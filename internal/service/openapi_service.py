...
class OpenAPIService(BaseService):
    ...
    提供基于开放API的聊天对话服务，支持流式和非流式响应。
    处理完整的对话流程，包括用户验证、会话管理、消息处理和智能体交互。
    """
    db: SQLAlchemy
    ...
    def chat(self, req: OpenAPIChatReq, account: Account):
        ...
        Args:
            req (OpenAPIChatReq): 开放API聊天请求参数
            account (Account): 当前账户信息
            
        Returns:
            Generator|Response: 流式响应生成器或响应对象
            
        Raises:
            NotFoundException: 当应用不存在或未发布时
            ForbiddenException: 当会话或用户权限不匹配时
        """
        # 1.判断当前应用是否属于当前账号
        ...
        if req.end_user_id.data:
            ...
            if not end_user or end_user.app_id != app_id:
            ...
            end_user = self.create(
                EndUser,
                **{"tenant_id": account_id, "app_id": app_id},
            )
            end_user_id = str(end_user.id)
